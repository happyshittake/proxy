...
import {Action, cleanUpNode, getNode, getNodeList, getRandNode, listen, Node} from "./discovery";
...
async function getProxy(url: string) {
    ...
    if (matchedProcessId && matchedProcessId[1]) {
        const node = await getNode(matchedProcessId[1]);
        if (node) {
            if (!processIds[node.processId]) {
                processIds[node.processId] = createProxy(node);
            }

            proxy = processIds[node.processId];
        }
    }
...
function createProxy(node: Node): httpProxy {
...
getNodeList().then(nodes => nodes.forEach(node => register(node))).catch(err => console.error(err));

const reqHandler = (req: http.IncomingMessage, res: http.ServerResponse) => {
    getProxy(req.url!).then(proxy => {
        if (proxy) {
            proxy.web(req, res);
        ...
        console.error("Err finding proxy!", processIds, err);
        res.statusCode = 503;
        res.end();
    });
};
